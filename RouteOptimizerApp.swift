...
final class RouteVM: ObservableObject {
    ...
    func addStop(name: String, address: String? = nil, lat: Double? = nil, lon: Double? = nil) {
        let displayName = name.isEmpty ? (address ?? "Arrêt \(stops.count + 1)") : name
        stops.append(Stop(name: displayName, address: address, latitude: lat, longitude: lon))
    }
    ...
    func importCSV(_ data: Data) {
        ...
        for line in lines {
            ...
            if p.count >= 3, let lat = Double(p[p.count - 2]), let lon = Double(p.last!) {
                let name = p.dropLast(2).joined(separator: ",")
                addStop(name: name, lat: lat, lon: lon)
            } else if p.count == 2, let lat = Double(p[0]), let lon = Double(p[1]) {
                addStop(name: "Arrêt \(stops.count + 1)", lat: lat, lon: lon)
            } else {
                let name = p.first ?? "Arrêt \(stops.count + 1)"
                let addr = p.dropFirst().joined(separator: ",")
                addStop(name: name, address: addr.isEmpty ? name : addr)
            }
        }
    }
        ...
        for i in stops.indices {
            ...
            do {
                ...
                if let c = placemarks.first?.location?.coordinate {
                    stops[i].latitude = c.latitude
                    stops[i].longitude = c.longitude
                }
            } catch {
                // on ignore les adresses introuvables
            }
    ...
    private func nearestNeighbor(_ pts: [Stop]) -> [Stop] {
        ...
        while !unvisited.isEmpty {
            ...
            for i in 0..<unvisited.count {
                let d = dist(cur.coordinate!, unvisited[i].coordinate!)
                if d < bestDist {
                    bestDist = d
                    bestIndex = i
                }
            }
            cur = unvisited.remove(at: bestIndex)
            route.append(cur)
        }
        return route
    }
        ...
        while changed {
            ...
            for i in 0..<(r.count - 2) {
                for j in (i + 2)..<r.count {
                    ...
                    let a = r[i].coordinate!
                    let b = r[i + 1].coordinate!
                    let c = r[j].coordinate!
                    let d = r[(j + 1) % r.count].coordinate!
                    let delta = (dist(a, c) + dist(b, d)) - (dist(a, b) + dist(c, d))
                    if delta < -1e-6 {
                        let mid = Array(r[(i + 1)...j].reversed())
                        r.replaceSubrange((i + 1)...j, with: mid)
                        changed = true
                    }
                }
            }
    ...
    func optimize() async {
        ...
        guard pts.count >= 2 else {
            status = "Ajoute au moins 2 arrêts."
            return
        }
        status = "Optimisation..."
    ...
    private func request(_ a: CLLocationCoordinate2D, _ b: CLLocationCoordinate2D) -> MKDirections.Request {
    ...
    func computeDirections() async {
        isBusy = true
        defer { isBusy = false }
        status = "Calcul d'itinéraires..."
        polylines.removeAll()
        totalDistance = 0
        totalTime = 0

        guard optimized.count >= 2 else {
            status = "Optimisé"
            return
        }
        ...
        for i in 0..<(optimized.count - 1) {
            if let a = optimized[i].coordinate, let b = optimized[i + 1].coordinate {
                do {
                    let resp = try await MKDirections(request: request(a, b)).calculate()
                    ...
                    // on ignore l'étape si ça échoue
                }
    ...
    func openInAppleMaps() {
        ...
        let items: [MKMapItem] = optimized.compactMap {
            guard let c = $0.coordinate else { return nil }
            let it = MKMapItem(placemark: MKPlacemark(coordinate: c))
            it.name = $0.name
            return it
        }
        MKMapItem.openMaps(
    ...
    func openInGoogleMaps() {
        ...
        var appURLString =
            "comgooglemaps://?directionsmode=driving&origin=\(origin)&destination=\(destination)"
        if !waypoints.isEmpty {
            appURLString += "&waypoints=\(waypoints)"
        }
        ...
        NavigationView {
            VStack(spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    HStack {
                        TextField("Nom (Client A)", text: $name)
                            .textFieldStyle(.roundedBorder)
                        TextField("Adresse ou lat,lon", text: $addr)
                            .textFieldStyle(.roundedBorder)
                    }
                        ...
                        ScrollView(.horizontal, showsIndicators: false) {
                            HStack {
                                ForEach(Array(sc.results.prefix(6).enumerated()), id: \.offset) { _, r in
                                    Button("\(r.title), \(r.subtitle)") {
                                        addr = "\(r.title), \(r.subtitle)"
                                        sc.results = []
                                    }
                    ...
                    HStack {
                        Button {
                            ...
                            if let latlon = parse(t) {
                                vm.addStop(name: name, lat: latlon.0, lon: latlon.1)
                            } else {
                                vm.addStop(name: name, address: t)
                            }
                            name = ""
                            addr = ""
                        } label: {
                ...
                List {
                    Section("Arrêts (\(vm.stops.count))") {
                        ForEach(vm.stops) { s in
                    ...
                    Section("Ordre optimisé") {
                        ForEach(Array(vm.optimized.enumerated()), id: \.offset) { i, s in
                            HStack {
                                Text("\(i + 1).")
